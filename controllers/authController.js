...
exports.register = async (req, res) => {
  try {
    const { IDnumber, gender, username, password } = req.body;

    let generatedID;
    if (!IDnumber || (IDnumber.length < 3 || IDnumber.length > 5) || isNaN(Number(IDnumber))) {
...
exports.login = async (req, res) => {
  try {
    ...
    res.status(200).json({ authToken, user: { id: user._id, username: user.username,Gender: user.gender } });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal server error' });
  }
...
exports.selectPlan = async (req, res) => {
  try {
    ...
    function checkMainMeals(mealTypes) {
      if (mealTypes?.includes('breakfast') && mealTypes?.includes('snack') && mealTypes?.length === 2) {
    ...
    // Send response
    res.json({ success: true, message: "data get successfully", data: combineData });
  } catch (error) {
    console.error(error);
...
exports.findNameOfAlimenti = async (req, res) => {
  try {
    ...
    console.log("🚀 ~ exports.findNameOfAlimenti= ~ foodGroupsArray:", foodGroupsArray)


    const alternatives = await Alimenti.aggregate([
      { $match: { 'List of Alternatives (Food group)': { $in: foodGroupsArray } } },
...
exports.datWiseMealPlan = async (req, res) => {
  try {
    ...
    if (mealData.length === 0) {
      return res.status(404).json({ error: `No meal plan found for the given parameters.` });
    }
    return res.json({ success: true, message: "meal plan data get successfully", data: mealData });
